...
class PhyCRNet(nn.Module):
    ...
    def __init__(self, input_channels, hidden_channels,
        ...
        self.fc74 = nn.Linear(30, 64)  # 第二层全连接层
    ...
    def forward(self, nid, bsize, forward_state,backward_state,x_t,x_tt,ref_speed,ref_speed1,chen0,x_t2,x_tt2):
        ...
        ref_source2 = torch.ones((1,2)).cuda()
        for step in range(0, bsize - 2, 1):
            c5 = torch.reshape(self.ref_sol2[step:step + 3, :, 1, :],(1,3*10))
            c52 = torch.reshape(self.ref_sol2[step:step + 3, :, :, 1], (1, 3 * 10))
            #c5 = torch.reshape(c5, (1, 3 *10 * 10))
            c5 = torch.tanh(self.fc74(c5))
            c52 = torch.tanh(self.fc174(c52))
            c5 = torch.tanh(self.fc75(c5))
            c52 = torch.tanh(self.fc75(c52))
            c2 = torch.reshape(torch.sigmoid(self.fc81(c5)), (1, 1))*64
            c52 = torch.reshape(torch.sigmoid(self.fc181(c52)), (1, 1)) * 64
            ref_source2[0,1] = (ref_source2[0,1] * step + c2) / (step + 1)
            ref_source2[0,0] = (ref_source2[0,0] * step + c52) / (step + 1)
        ...
        x_t = 1e3 * torch.exp(-((X2 - ref_source2[0, 0]) ** 2 + (Y2 - ref_source2[0, 1]) ** 2) / 100)
        x_t = torch.unsqueeze(torch.unsqueeze(x_t, dim=0), dim=0)
        x_tt = x_t
        ...
        for step in range(0, self.step2 - 1, 1):
            ...
            x_temp3[:, :, 1:-1, 1:-1] = (2 * x_t[:, :, 1:-1, 1:-1] - x_tt[:, :, 1:-1, 1:-1]) + x_temp4[:, :, 1:-1,
                                                                                               1:-1] * \
                                        (ref_speed3[:, :, 1:-1, 1:-1] ** 2) * (self.dt ** 2) / (self.dx ** 2)
        ...
        return outputs11, outputs1, outputs6, outputs62, second_last_state_forward, second_last_state_backward, ref_speed, ref_speed1, ref_source2
...
class loss_generator(nn.Module):
    ...
    def get_phy_Loss3(self, output,c,bsize,id):
        ...
        r1, r2 = ((dt**2)*(c[:,1:-1,1:-1]**2))/ (dx ** 2),((dt**2)*(c[:,1:-1,1:-1]**2))/ (dy ** 2)
        for n in range(1, int(t_max / dt)):
            # 在边界处设置固定边界条件
...
def compute_loss(batchloss,output1, output9,output4,output8,loss_func, id, ref_speed,ref_source2,chen0,bsize,ntb,epoch,omega1,omega2,omega3,omega4,omega11,omega21,omega31,omega41):
    ...
    ref_speed2 = 45*torch.ones((64,64)).cuda()
    #
    ...
    # output13 = loss_func.get_phy_Loss3(output4, ref_speed2, bsize, id)
    ...
    w1 = []
    for i in range(int(output11.shape[0])):
        w1.append(((bsize - 2) * (ntb - id) - i) / ((bsize - 2) * ntb))

    for i in range(output11.shape[0]):
        p_local += w1[i]*mse_loss(p_local36[i+2:i + 3, :,3, :],
...
def train(model, input, ref_res, forward_state, backward_state, n_iters, time_batch_size, learning_rate,
    ...
    for epoch in range(n_iters):
        ...
        for time_batch_id in range(num_time_batch):
            ...
            # output is a list
            output1,output2,output3,output4,second_last_state_forward,second_last_state_backward, ref_speed,ref_speed1, chen0= model(time_batch_id,time_batch_size,hidden_state1,hidden_state2,x_t,x_tt,ref_speed,ref_speed1,chen0,x_t2,x_tt2)
        ...
        # with torch.autograd.set_detect_anomaly(True):
            loss, loss2, loss3, loss4, loss5, loss6= compute_loss(batch_loss,output1,output2,output3,output4,loss_func,time_batch_id,
                                                                         ref_speed,ref_speed1,chen0,time_batch_size,num_time_batch,epoch,omega1,omega2,omega3,omega4,omega11,omega21,omega31,omega41)
        ...
        locx = chen0[0:1, 0:1].item()
        locy = chen0[0:1, 1:2].item()
        # print loss in each epoch
        print('[%d/%d %d%%] loss: %.5f loc: (%.5f,%.5f) loss_true: %.5f loss_res: %.5f loss_boundary: %.5f loss_init: %.5f loss_local: %.5f'  % ((epoch + 1), n_iters, ((epoch + 1) / n_iters * 100.0),
                                            batch_loss,locx,locy, batch_loss2, batch_loss3, batch_loss4, batch_loss5, batch_loss6))
        train_loss_list.append(batch_loss)
        #ref_speed_arr.append(ref_speed1[0:1,0:1].item())
        source_loc_list_x.append(locx)
        source_loc_list_y.append(locy)
